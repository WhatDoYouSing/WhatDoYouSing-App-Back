...
from .models import Notes, NoteComment, NoteEmotion, Emotions, Plis
...
class NoteDetailView(APIView):
    ...
    def get(self, request, note_id):
        ...
        # 태그 가져오기
        tags = list(note.tag_time.values_list("name", flat=True)) + \
        ...
        friends = UserFollows.objects.filter(
            Q(follower=user) & 
            Q(following__in=UserFollows.objects.filter(following=user).values_list('follower', flat=True))
        ...
        same_user_notes = Notes.objects.filter(user=note.user, visibility__in=visibility_filter).exclude(id=note.id).order_by('-created_at')[:2]
        same_user_plis = Plis.objects.filter(user=note.user, visibility__in=visibility_filter).order_by('-created_at')[:2]
        same_user = list(NoteSerializer(same_user_notes, many=True).data) + list(PliSerializer(same_user_plis, many=True).data)
        same_user = sorted(same_user, key=lambda x: x['created_at'], reverse=True)[:2]

        # 이 노래의 다른 노트 보기 =====================================

        filtered_notes = Notes.objects.filter(
            Q(user=user, song_title=note.song_title, artist=note.artist) |  #  내 노트는 다 포함
            Q(user__in=friends, visibility='friends', song_title=note.song_title, artist=note.artist) |  # 친구면 friends 포함
            Q(visibility='public', song_title=note.song_title, artist=note.artist)  #  일반 사용자는 public만
        ).exclude(id=note.id).order_by('-created_at')[:2]
        ...
        related_pli_ids = PliNotes.objects.filter(
            notes__song_title=note.song_title,
            notes__artist=note.artist
        ).values_list('plis', flat=True)

        # 플리 필터링 (visibility 적용)
        same_pli = PliSerializer(
            Plis.objects.filter(id__in=related_pli_ids, visibility__in=visibility_filter).order_by('-created_at')[:2], 
            many=True
        ).data
...
class NoteEmotionToggleView(APIView):
    ...
    def post(self, request, note_id, emotion_name):
        ...
        # 기존 감정이 있는지 확인
        existing_emotion = NoteEmotion.objects.filter(note=note, user=user, emotion=emotion).first()
...
class SameUserContentsView(APIView):
    ...
    def get(self, request, user_id):
        ...
        if user_id == user.id:
            ...
            # 친구: 공개 + 친구 공개 콘텐츠만 조회
            notes = Notes.objects.filter(user=user_owner, visibility__in=['public', 'friends'])
        ...
        # 직렬화 및 최신순 정렬
        combined_data = list(NoteSerializer(notes, many=True).data) + list(PliSerializer(plis, many=True).data)
...
class SameSongNoteView(APIView):
    ...
    def get(self, request, note_id):
        ...
        filtered_notes = Notes.objects.filter(
            ...
            Q(user__in=friends, visibility='friends', song_title=note.song_title, artist=note.artist) |  # ✅ 친구면 friends 포함
            Q(visibility='public', song_title=note.song_title, artist=note.artist)  # ✅ 일반 사용자는 public만
        ).exclude(id=note.id).order_by('-created_at')
...
class SameSongPliView(APIView):
    ...
    def get(self, request, note_id):
        ...
        # 플리 필터링 (visibility 적용)
        filtered_plis = Plis.objects.filter(id__in=related_pli_ids).filter(
            Q(user=user) |  # 본인 → 모든 플리 포함
            Q(user__in=friends, visibility__in=['public', 'friends']) |  # 친구 → friends + public 포함
...
class NoteCommentView(APIView):
    ...
    def post(self, request, note_id):
        ...
        if comment_id:
            ...
            return Response(
                {"message": "댓글이 등록되었습니다.", "data": NoteCommentSerializer(comment).data},
                status=status.HTTP_201_CREATED
            )
