...
class PlaylistDetailView(BlockFilterMixin, APIView):
    ...
    def get(self, request, pk):
        ...
        is_friend = (
            ...
            and UserFollows.objects.filter(follower=pli_owner, following=user).exists()
        )

        # 접근 권한 체크 (본인/친구/타인)
        if (
            pli_owner == user
            or pli.visibility == "public"
            or (is_friend and pli.visibility == "friends")
        ):
            can_access = True
        else:
            return Response(
                {"error": "이 플리를 볼 수 없습니다."}, status=status.HTTP_403_FORBIDDEN
            )
        ...
        for pli_note in pli_notes:
            ...
            if (
                ...
                or (is_friend and note.visibility == "friends")
            ):
        ...
        # 태그 리스트 가져오기
        tags = (
            list(pli.tag_time.values_list("name", flat=True))
        ...
        is_collected = ScrapPlaylists.objects.filter(
            scrap_list__user=user, content_id=pli.id
        ).exists()
        ...
        pli_data = {
            ...
            "created_at": timezone.localtime(pli.created_at),
        ...
        return Response(
            {"message": "플리 상세 조회 성공", "data": pli_data},
...
class SameUserPliView(APIView):
    ...
    def get(self, request, user_id):
        ...
        return Response(
            {"message": message, "data": serialiser_data}, status=status.HTTP_200_OK
        )
            ...
            return Response(
                {
                    ...
                    "data": PliReplySerializer(reply).data,
                },
                status=status.HTTP_201_CREATED,
            )
        else:
            # 일반 댓글 작성
            comment = PliComment.objects.create(pli=pli, user=user, content=content)
            return Response(
                {
                    "message": "댓글이 등록되었습니다.",
                    "data": PliCommentSerializer(comment).data,
                },
                status=status.HTTP_201_CREATED,
            )
