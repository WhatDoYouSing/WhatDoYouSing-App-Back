    ...
    def get(self, request):
        ...
        if is_writer_search:
            ...
            if writer.startswith("from:@"):
                ...
                writer_qs = User.objects.filter(serviceID=username)
            ...
            # `@닉네임` → 닉네임 또는 아이디 추출
                ...
                writer_qs = User.objects.filter(Q(serviceID=uname) | Q(nickname=uname))
                if not writer_qs.exists():
                    return Response(
                        {"message": "해당 사용자가 존재하지 않습니다."},
                        status=status.HTTP_404_NOT_FOUND,
    ...
    def get(self, request):
        ...
        if writer:
            if writer.startswith("from:@"):
                ...
                try:
                    user = User.objects.get(serviceID=username)  # ✅ ID(username)만 검색
                    user_query = Q(user=user)
                    has_filter = True
                except User.DoesNotExist:
                    return Response(
                ...
                try:
                    user = User.objects.get(
                        Q(serviceID=username_or_nickname)
                        | Q(nickname=username_or_nickname)
                    )
                    user_query = Q(
                        user=user
